...
/*
 * Type for a VAD instance, an opaque object created using wvad_new().
 */
typedef struct Wvad Wvad;
 ...
 * eventually be deleted using wvad_free().
 *
 ...
 */
int wvad_set_mode(Wvad* inst, int mode);
 ...
 */
int wvad_set_sample_rate(Wvad* inst, int sample_rate);
 ...
 */
int wvad_process(Wvad* inst, const int16_t* frame, size_t length);
